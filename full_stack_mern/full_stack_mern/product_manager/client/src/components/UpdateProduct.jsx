...
const UpdateProduct = ( { addNewProduct } ) => {
    ...
    useEffect(() => {
        axios.get(`http://localhost:8000/api/products/${id}`)
            .then((res) => {
                ...
                setInputObject({...product, price: product.price.slice(0, -1)});
            })
            .catch((err) => {
                console.log(err);
            })
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        axios.patch(`http://localhost:8000/api/products/${id}`, {...inputObject, price: inputObject.price + '$'})
            .then( result => {
    ...
    return (
        <div className='w-50 mx-auto my-4'>
            ...
            <form className='form p-4 d-flex flex-column align-items-center' onSubmit={handleSubmit}>
                ...
                <input type="submit" value="Update" className='btn btn-primary w-25' />
            </form>
        </div>
    );
}
